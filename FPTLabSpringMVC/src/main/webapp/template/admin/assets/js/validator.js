...
Validator.isDateAfter = function(selector, message, compareDate) {
    return {
        selector: selector,
        test: function(value) {
            // Chuyển đổi ngày nhập vào và ngày so sánh thành đối tượng Date
            var inputDate = parseDate(value);
            var compareToDate = parseDate(compareDate);

            // Kiểm tra nếu ngày nhập vào lớn hơn ngày so sánh
            return inputDate > compareToDate ? null : message || 'Ngày phải lớn hơn ngày '+compareDate;
        }
    };
